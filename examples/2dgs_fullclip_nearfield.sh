#!/usr/bin/env bash
# Targeted near-field static 2DGS experiment.
#
# The base 11 tiles cover the whole clip.  Nine extra -12 degree pitch tiles
# (front-wide/cross-left/cross-right × -20/0/+20 yaw) move nearby road and
# target-FOV side content away from the source-image boundary.  They share
# the same FTheta sampling, dynamic-mask exclusion and multi-scan LiDAR
# targets as the base tiles.
set -euo pipefail

PHASE=${1:?usage: $0 build|train|evaluate|render}
...
DATASET=/data/zrz/Cross-Vehicle/outputs/2dgs/7fd4d554/chunk0-clean-static-road-nearfield-proxy-5cam-20tile-299f-480-200k-v1
MODEL=/data/zrz/Cross-Vehicle/outputs/2dgs/7fd4d554/chunk0-clean-static-road-nearfield-fullclip-20tile-12000-v1
RENDER=/data/zrz/Cross-Vehicle/outputs/2dgs/7fd4d554/l4-front-center-clean-static-road-nearfield-sky-fullclip-20tile-diagnostic-v1
BEST_ITER=${BEST_ITER:-12000}

export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1}
export TORCH_CUDA_ARCH_LIST=${TORCH_CUDA_ARCH_LIST:-8.9}
export MAX_JOBS=${MAX_JOBS:-4}
export MPLCONFIGDIR=${MPLCONFIGDIR:-/tmp/mpl-2dgs-nearfield}
...
case "$PHASE" in
  build)
    ...
    PYTHONPATH=${PROJECT_ROOT}/src conda run --no-capture-output -n instant-nurec \
      ...
      --near-field-camera-ids camera_front_wide_120fov,camera_cross_left_120fov,camera_cross_right_120fov \
      --near-field-tile-yaws-deg=-20,0,20 --near-field-tile-pitch-deg=-12 \
  ...
  train)
    ...
    PYTHONPATH=$PWD conda run --no-capture-output -n instant-nurec \
      ...
      --iterations 12000 --eval --lambda_dssim 0 --lambda_dist 10 --lambda_normal 0.05 \
      --lambda_road_depth 0.20 --lambda_road_normal 0.02 --road_depth_start_iter 500 \
      --densify_until_iter 3000 \
      --test_iterations 3000 6000 8000 10000 12000 \
      --save_iterations 3000 6000 8000 10000 12000 \
      --checkpoint_iterations 3000 6000 8000 10000 12000
    ;;
  evaluate)
    for iter in 3000 6000 8000 10000 12000; do
      PYTHONPATH=${PROJECT_ROOT}/src:${TWO_DGS_ROOT} conda run --no-capture-output -n instant-nurec \
        ...
        --iteration "$iter" --split holdout \
        --output "$MODEL/road_depth_holdout_${iter}.json" --device cuda
    done
    ;;
  render)
    PYTHONPATH=${PROJECT_ROOT}/src:${TWO_DGS_ROOT} conda run --no-capture-output -n instant-nurec \
      ...
      --output "$RENDER" --iteration "$BEST_ITER" --camera-id front_center --chunk-index 0 \
    ...
    conda run --no-capture-output -n instant-nurec ffmpeg -y -framerate 30 -start_number 0 \
      ...
      "$RENDER/front_center/l4_front_center_static_2dgs_road_nearfield_sky_fullclip_h264.mp4"
    ;;
  *)
    echo "unknown phase: $PHASE (expected build, train, evaluate or render)" >&2
    exit 2
    ;;
esac
